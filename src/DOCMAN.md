...
| --- | --- | --- |
| `-directory` | Yes | Directory containing files to process. |
| `-csv` | Yes | CSV file path used for output. |
| `-search` | Yes | Search query used to find matching files. |
| `-recursive` | Yes | When used with [`search`](SEARCH.md), includes files in subfolders. |
| `-extensions` | Yes | File extensions to include. |
| `-updaterefs` | No | Reserved parameter for reference update behavior. |
