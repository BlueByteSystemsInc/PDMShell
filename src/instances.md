...
âœ… launch PDMShell as a single instance controller  
âœ… allow subsequent commands to reuse the same PDMShell instance  
âœ… improve performance if triggered from `cmd.exe` or `Dispatch`   
âœ… prevent multiple conflicting PDMShell instances
...
PDMShell can also detect a `.pdmshell` script path on the command line and run it through [`runscript`](RUNSCRIPT.md).
